-- Comm.lua: envelope version stamping and the sync-compatibility gate,
-- driven through the real send/receive path with stubbed Ace plumbing.
...
-- Own sends are stamped and (as echoes) dispatch fine.
assert(ns.Comm.Send("SYSTEM", { x = 1 }))
assert(wire.sent.ver == "0.1.0", "Send did not stamp the version")
wire.receive("Parchment", wire.sent, "PARTY", "Alice")
assert(#got == 1 and got[1][1].x == 1)
assert(ns.Comm.Whisper("SYSTEM", {}, "Bob") and wire.sent.ver == "0.1.0", "Whisper did not stamp")

-- Patch drift syncs; minor drift at major 0 does not.
wire.receive("Parchment", { t = "SYSTEM", v = {}, ver = "0.1.9" }, "PARTY", "Bob")
assert(#got == 2, "patch difference wrongly blocked")
wire.receive("Parchment", { t = "SYSTEM", v = {}, ver = "0.2.0" }, "PARTY", "Carol")
...
wire.receive("Parchment", { t = "SYSTEM", v = {}, ver = "0.0.9" }, "PARTY", "Dave")
...
wire.receive("Parchment", { t = "SYSTEM", v = {} }, "PARTY", "Eve")
wire.receive("Parchment", { t = "SYSTEM", v = {}, ver = "1.0.0" }, "PARTY", "Frank")
assert(#got == 2)

-- Wrong prefix and garbage envelopes are ignored without errors.
wire.receive("OtherAddon", { t = "SYSTEM", v = {}, ver = "0.1.0" }, "PARTY", "Gail")
wire.receive("Parchment", "not-a-table", "PARTY", "Gail")
assert(#got == 2)
...
wire2.receive("Parchment", { t = "SYSTEM", v = {}, ver = "1.5.3" }, "PARTY", "Hank")
assert(got2 == 1, "1.x minor drift wrongly blocked")
wire2.receive("Parchment", { t = "SYSTEM", v = {}, ver = "2.0.0" }, "PARTY", "Iris")
